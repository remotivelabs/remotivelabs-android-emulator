...
def get_devices(adb_path='adb'):
    with open(os.devnull, 'wb') as devnull:
        subprocess.check_call([adb_path, 'start-server'], stdout=devnull,
                              stderr=devnull)
    out = split_lines(subprocess.check_output([adb_path, 'devices'], encoding='UTF-8'))
...
def _get_device_by_type(flag, adb_path):
    ...
    try:
        serial = subprocess.check_output(
            [adb_path, flag, 'get-serialno'], encoding='UTF-8').strip()
...
class AndroidDevice(object):
    ...
    def _simple_call(self, cmd):
        return subprocess.check_output(self.adb_cmd + cmd, encoding='UTF-8')
    ...
    def get_sensor_data(self, sensor_name):
        return self._simple_call(['emu', 'sensor', 'get', sensor_name])

    def get_sensors_list(self):
        return self._simple_call(['emu', 'sensor', 'status'])
