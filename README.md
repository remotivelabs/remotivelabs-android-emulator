...
Include the credentials from the broker (URL and API key). You can find the broker credentials in the bottom left corner once you have prepared the recording for playback.
...
If your signal transformation in RemotiveCloud uses different names for the location signals they can be set using `--longitude-signal-name` and `--latitude-signal-name`.
...
This will send try to subscribe on all the signals listed in the mappings file and send them to the android emulator. If you are only interested in a limited number of signals you can supply which signals to subscribe on. They must still exist within the mappings json file.
...
#### Both Location and VHAL properties

You can also include both location data and vhal properties by including arguments for both.
