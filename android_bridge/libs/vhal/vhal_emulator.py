...
class Vhal:
    ...
    def open_socket(self, device=None):
        ...
        remotePortNumber = 33452
        # Hard-coded to allow socat port redirect
        localPortNumber = 33452
        extraArgs = "" if device is None else "-s %s" % device
        adbCmd = "adb %s forward tcp:%d tcp:%d" % (
            extraArgs,
            localPortNumber,
            remotePortNumber,
        )
        ...
        print(
            "Connecting local port %s to remote port %s on %s"
            % (
                localPortNumber,
                remotePortNumber,
                "default device" if device is None else "device %s" % device,
    ...
    #                 print("Received proto message:", msg)
    #                 return msg
    #             else:
